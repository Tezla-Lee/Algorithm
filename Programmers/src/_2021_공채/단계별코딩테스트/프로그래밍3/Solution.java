package _2021_공채.단계별코딩테스트.프로그래밍3;
...
class Solution {
    ...
    public boolean[] solution(String program, String[] flag_rules, String[] commands) {
        ...
        for (String flag_rule : flag_rules) {
            ...
            if (flag_rule_split.length == 2) {
                rules.put(flag_rule_split[0], flag_rule_split[1]);
            } else {
                aliasList.add(flag_rule_split);
            }
        }

        for (String[] flag_rule_split : aliasList) {
            rules.put(flag_rule_split[0], rules.get(flag_rule_split[2]));
        }

        for (int i = 0; i < commands.length; i++) {
            answer[i] = isCorrectCommand(commands[i], program, aliasList); // command 를 하나씩 확인하고 answer 배열에 저장
        }

        return answer;
    }

    public boolean isCorrectCommand(String command, String program, List<String[]> aliasList) {
    ...
    public boolean validate(String[] command_split, int flag_argument_type_index, String word) {
    ...
    // 별칭과 함께 사용하는지 체크하는 메소드
    public boolean isOverlap(List<String[]> aliasList, String command) {

        for (String[] aliasCommand : aliasList) {
            if (command.contains(aliasCommand[0]) && command.contains(aliasCommand[2])) {
                return true; // 별칭 같이 사용 하면 true
            }
        }
    ...
    public static void main(String[] args) {
        System.out.println(Arrays.toString(new Solution().solution("line", new String[]{"-s STRING", "-num NUMBER", "-e NULL", "-n ALIAS -num"}, new String[]{"line -n 100 -s hi -e", "line -n 100 -e -num 150"})));
        System.out.println(Arrays.toString(new Solution().solution("bank", new String[]{"-send STRING", "-a ALIAS -amount", "-amount NUMBERS"}, new String[]{"bank -send abc -amount 500 200 -a 400", "bank -send abc -a hey"})));
    }
}